...
- **Structured concurrency only, zero Combine** — async/await end to end. Shared mutable state lives in actors (repositories, stores), or behind a `Mutex` where registration has to be synchronous — cancelling a task you have not finished registering is a race, not a hop; UI-facing types are `@MainActor`. The project compiles under **Swift 6 strict concurrency**, so data-race safety is enforced by the compiler rather than by convention.
